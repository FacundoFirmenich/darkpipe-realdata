...
def _partition(
    ...
    tiles: list[str],
    *,
    orientation_basis: bool = False,
) -> None:
    sums = empty_pair_sums(2, 3)
    if orientation_basis:
        sums.update({key: np.full((2, 3), value) for key in ORIENTATION_BASIS_KEYS})
    ...
    save_pair_partition(
        ...
        sums,
        {
            ...
            "orientation_basis_included": orientation_basis,
            "source_tiles": tiles,
...
def test_orientation_basis_is_merged_without_losing_hash_authority(tmp_path: Path) -> None:
    first, second = tmp_path / "a.npz", tmp_path / "b.npz"
    _partition(first, 0, 4, 1.0, ["A"], orientation_basis=True)
    _partition(second, 4, 10, 2.0, ["B"], orientation_basis=True)
    sums, metadata = merge_partitions([first, second], require_complete_surface=False)
    assert metadata["orientation_basis_included"] is True
    for key in ORIENTATION_BASIS_KEYS:
        np.testing.assert_array_equal(sums[key], np.full((2, 3), 3.0))
